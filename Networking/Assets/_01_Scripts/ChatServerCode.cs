...
namespace ChatClientExample
{
    ...
    public class ChatServerCode : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            for (int i = 0; i < m_Connections.Length; i++)
            {
                if (!m_Connections[i].IsCreated)
                {
                    ...
                    {
                        chat.NewMessage($"{nameList[m_Connections[i]]} has disconnected.");
        ...
        static void HandleClientHandshake(object handler, NetworkConnection connection, DataStreamReader stream)
        {
            ...
            ChatServerCode serv = handler as ChatServerCode;

            // Add to list
            serv.nameList.Add(connection, str.ToString());
            serv.chat.NewMessage($"{str.ToString()} has joined the chat.");
        ...
        {
            // Pop message
            FixedString512Bytes str = stream.ReadFixedString512();

            ChatServerCode serv = handler as ChatServerCode;

            if (serv.nameList.ContainsKey(connection))
            {
                serv.chat.NewMessage($"{serv.nameList[connection]}: {str.ToString()}");
            }
            else
            {
        ...
        static void HandleClientExit(object handler, NetworkConnection connection, DataStreamReader stream)
        {
            ...
            if (serv.nameList.ContainsKey(connection))
            {
                serv.chat.NewMessage($"{serv.nameList[connection]} has left the chat.");
